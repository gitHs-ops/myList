...
- **`car-maintenance.html`·`hyundai-proxy/`의 변경은 Claude가 커밋 대상에서 항상 제외한다** (2026-09-14, 사용자 지시) — 이 두 파일/폴더는 사용자가 별도로 진행 중인 작업이라 `git add -A` 등으로 쓸어담지 말고, 요청받은 파일만 콕 집어 add할 것. 이 저장소에서 커밋+푸시하기 전에 `git status`로 이 두 경로가 끼어 있지 않은지 항상 확인한다. **다만 매번 "제외했습니다" 식으로 보고하지 말 것**(2026-09-14) — 조용히 지키기만 하면 됨, 별도 언급 불필요.
