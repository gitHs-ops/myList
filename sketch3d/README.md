...
그 자리에 배치되고(연속 배치 가능, 버튼 다시 누르면 취소), 배치된 가구를 클릭하면 제거된다.
1인칭 시점과는 동시에 켤 수 없다(가구 버튼을 누르면 1인칭이 꺼지고, 1인칭을 켜면 배치 모드가
꺼진다). 회전은 아직 없음(항상 기본 방향의 육면체). 모델을 다시 만들어도(샘플 선택, JSON
재생성 등) 배치는 유지된다. 타입별 기본 크기는 `index.html` 의 `FURNITURE_TYPES`. 단위 mm.
