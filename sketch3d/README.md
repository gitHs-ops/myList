...
3D 뷰(`.viewport-card`) 상단 "🛋️ 가구 배치" 줄에서 타입 버튼을 고른 뒤 바닥을 클릭하면
그 자리에 배치되고(연속 배치 가능, 버튼 다시 누르면 취소, "✋ 배치 해제"로도 끌 수 있음),
배치된 가구를 클릭하면 배치 모드 여부와 무관하게 항상 제거된다. 벽 바깥(벽 두께 안쪽까지
포함)을 클릭하면 무효 처리된다 — 정확한 방 폴리곤이 아니라 전체 벽 바운딩박스를 벽 두께만큼
줄인 사각형으로 근사하므로, 오목한(L자) 평면에서는 완벽하지 않다. 1인칭 시점과는 동시에 켤
수 없다(가구 버튼을 누르면 1인칭이 꺼지고, 1인칭을 켜면 배치 모드가 꺼진다). 회전은 아직
없음(항상 기본 방향의 육면체). 모델을 다시 만들어도(샘플 선택, JSON 재생성 등) 배치는
유지된다. "🗑 전체 삭제"로 한번에 정리 가능. 타입별 기본 크기는 `index.html` 의
`FURNITURE_TYPES`. 단위 mm.
